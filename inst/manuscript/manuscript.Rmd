...
We may also predict values of the latent mean (on the link scale) at new locations by leveraging the spatial covariance between observed locations and new locations. Again suppose that we observed $\mathbf{w}$ and we want to make predictions at $\mathbf{u}$, a vector of latent means at the new locations that follows the same SPGLM from Equation~\ref{eq-spglm} with fixed effects design matrix, $\mathbf{X}_{\mathbf{u}}$. The vector $(\mathbf{w}, \mathbf{u})^\top$ has the following properties:
\begin{align}
...
\end{align}
Two more sources of uncertainty, $\hat{\boldsymbol{\beta}}$ and $\hat{\mathbf{w}}$.
Spatial prediction is often also called Kriging [@cressie1990origins]. 
