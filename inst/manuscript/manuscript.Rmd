...
spnb_anis <- update(sppois_anis, family = nbinomial)
```
...
tidy(anova(spbeta_full_ml, spbeta_red_ml))
```
...
```{r}
pairs(emmeans::emmeans(spgam, ~ state | temp))
emmeans::emtrends(spgam, ~ state, var = "temp")
```
