...
\citep{lenth2024emmeans}, and have tools for model summaries,
prediction, and simulating data. There are some notable differences
between the two packages, however. \pkg{sdmTMB} supports several
additional GLM distributions like the Tweedie, supports Hurdle models,
and can incorporate prior information through Bayesian applications.
\pkg{sdmTMB} also provides tools for working with temporal data and
enhanced visualizations of marginal effects. \pkg{sdmTMB} does require a
preprocessing step of constructing a mesh for the stochastic partial
differential equation approach, and the density of the mesh can affect
model results and computational complexity. \pkg{spmodel} does not
...
covariance parameters (\(\boldsymbol{\theta}\)). Following
@\citet{harville1977maximum}, we can integrate \(\boldsymbol{\beta}\)
out of Equation\(~\)\ref{eq-spglm}, which yields
...
locations and new locations. Again suppose that we observed
...
\end{align} Two more sources of uncertainty,
\(\hat{\boldsymbol{\beta}}\) and \(\hat{\mathbf{w}}\). Spatial
prediction is often also called Kriging \citep{cressie1990origins}.

\section{Modeling moose presence in Alaska, USA}\label{sec-applications}

\subsection{Additional applications}\label{sec-applications2}

\subsection{Modeling moose counts in Alaska,
USA}\label{modeling-moose-counts-in-alaska-usa}

\subsection{Modeling harbor seal trends in Alaska,
USA}\label{modeling-harbor-seal-trends-in-alaska-usa}

\subsection{Modeling voter turnout in Texas,
USA}\label{modeling-voter-turnout-in-texas-usa}

\subsection{Modeling lake conductivity in Southwest,
USA}\label{modeling-lake-conductivity-in-southwest-usa}
