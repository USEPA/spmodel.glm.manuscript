...
less intuitive than working directly with the covariance matrix.

The \pkg{FRK} \citep{sainsbury2024modeling}, \pkg{glmmTMB}
...
reasons. First, it is general enough that it can be applied to any
...
models). For both point-referenced data and areal data, \pkg{spmodel}
supports the binomial distribution for binary data, Poisson and negative
binomial distributions for count data, Gamma and inverse Gaussian
distributions for skewed data, and the beta distribution for proportion
data. There are 20 different spatial covariance structures available
including the exponential, Gaussian, and spherical for point-referenced
data (Figure\(~\)\ref{fig-type}) and the conditional autoregressive, and
simultaneous autoregressive structures for areal data. \pkg{spmodel}
provides tools for commonly used model summaries, visualizations, and
diagnostics (e.g., Cook's distance) using standard \proglang{R} helper
functions like \code{summary()}, \code{plot()}, \code{fitted()}, and
\code{tidy()}, among others. \pkg{spmodel} also provides tools to
predict \(\mathbf{w}\) at new locations and quantify uncertainty in
those prediction using \code{predict()} and \code{augment()}. This core
functionality, combined with several advanced features we describe
throughout the manuscript, enables \pkg{spmodel} to introduce novel,
important SPGLM modeling tools previously missing from the existing
...
0.11.0) is arguably most similar to \pkg{sdmTMB} (version 0.7.4) in
...
constructing a mesh for the stochastic partial differential equation
...
covariance as in \pkg{sdmTMB}. \pkg{spmodel} can model data directly
using neighborhood distance and autoregressive models, rather than
relying on the polygon centroid (as in \pkg{sdmTMB}), which may not be
within the polygon's boundaries. \pkg{spmodel} provides experimental
...
distances, and has analytic solutions for fixed effect and prediction
standard errors. Other similarities and differences do exist between
\pkg{sdmTMB} and \pkg{spmodel}, and both packages continue to evolve.
Overall, we believe that these packages are complementary and enhance
the suite of SPGLM tools accessible to practitioners.
...
estimation, inference, model diagnostics, and prediction. Then in
Section\(~\)\ref{sec-applications} and
Section\(~\)\ref{sec-applications2}, we show how to use \pkg{spmodel} to
carry out these primary data analysis tasks for various data sets.
...
\([\mathbf{w} | \mathbf{X}, \boldsymbol{\beta}, \boldsymbol{\theta}]\)
share spatial covariance. Following \citet{harville1977maximum}, we can
...
\(\hat{\mathbf{w}}\). Maximizing this log likelihood requires repeatedly
evaluating \(\boldsymbol{\Sigma}^{-1}\), \(\mathbf{g}\), and
\(\mathbf{G}\); see \citet{ver2024marginal} for more details and
comparisons to other approaches as well as and forms of \(\mathbf{g}\)
and \(\mathbf{G}\) for various response distributions.
...
\code{family}, the response distribution assumed for the response
...
covariance parameters, and the GLM dispersion parameter (fixed at one in
logistic regression):
...
covariance matrix (the primary computational burden), while fitting
...
(\code{.cooksd}), and standardized residuals \code{.std.resid} described
in Section\(~\)\ref{sec:diagnostics}. A benefit of using
...
variance, and residual nonspatial variance contribute to model fit, but
as with \(PR^2\), should not be used as a model comparison tool.
...
fit contrasts for models with interactions. In
...
Table\(~\)\ref{tab:additional} outlines the section number, data set,
family (i.e., response distribution), geometry type, and additional
\pkg{spmodel} features for each application.
...
custom neighborhood matrices can are passed via \code{W}. Row
...
In contrast, ML estimators are biased, though in practice this bias may
be small. Moreover, when using ML, likelihood-based comparisons are
valid for models having different explanatory variable and fixed effect
structures. Using ML, we can evaluate the significance of log income on
voter turnout with a likelihood ratio test:
...
related to voter turnout (\(p\)-value \textless{} 0.001). Alternatively,
we could have instead used a different likelihood-based statistic like
AIC:
